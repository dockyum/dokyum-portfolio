...
describe("ProjectRunway", () => {
  ...
  it("captions each card with its name, overview, and my role, tagging independent work only", () => {
    const { container } = render(<ProjectRunway projects={projects} />);
    for (const project of projects) {
      const card = within(primaryCard(project.name));
      const caption = card.getByText(project.overviewLine).closest(".project-card-caption")!;
      expect(caption).toHaveTextContent(project.name);
      expect(caption).toHaveTextContent(project.role);
